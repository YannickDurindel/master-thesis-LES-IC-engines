...
\begin{enumerate}
    ...
    \item \textbf{Boundary layer is the target:} The purpose of this study is specifically to improve near-wall predictions for heat transfer calculations. A small deviation in bulk velocity is an acceptable trade-off for significantly improved boundary layer accuracy.
...
In summary, the selection of $C_m = 4.0$ prioritizes boundary layer accuracy over bulk flow matching, which is the appropriate choice for wall heat transfer studies in internal combustion engines.
...
\begin{itemize}
    ...
    \item The selection of $C_m = 4.0$ prioritizes boundary layer accuracy, which is critical for engine heat transfer predictions. The resulting deviation in centerline velocity does not impact engine simulations due to geometric and flow structure differences.
